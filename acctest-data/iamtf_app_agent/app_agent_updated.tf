...
resource "iamtf_app_agent" "test" {
  ...
  ida                      = iamtf_identity_appliance.test.name
  app_slo_location         = "http://myapp-updated-replace_with_uuid:8080/partnerapp/slo"
  app_location             = "http://myapp-updated-replace_with_uuid:8080/partnerapp"
  ignored_web_resources    = ["*.ico"]
  default_resource         = "http://myapp-updated-replace_with_uuid:8080/partnerapp/home"
  ...
  saml2 {
      message_ttl           = 401
      message_ttl_tolerance = 411

      sign_authentication_requests = true
      //sign_requests                 = true
      signature_hash       = "SHA-384"
